...
from scripts.utils import hit_rate_at_k, precision_at_k, reciprocal_rank
...
df_save_results = pd.DataFrame(
    columns=["query", "context", "provided_answer", "scoring", "id"]
)
print("Data Loaded!")
...
if tokenizer.pad_token is None:
    ...
    if (
        hasattr(model_cpu, "generation_config")
        and model_cpu.generation_config is not None
    ):  # Check if generation_config exists
...
for i in tqdm(
    range(num_questions_to_run),
    desc="Processing Questions",
    disable=not accelerator_main.is_main_process,
):
    ...
    utility_cache_base_dir = f"Shuffle_data/utilities_cache3bcp"
    utility_cache_filename = (
        f"utilities_shuffle_q_idx{i}_n{len(docs)}.pkl"  # More robust naming
    )
    ...
    if accelerator_main.is_main_process:  # Only main process creates directories
        ...
        print(
            f"  Instantiating ShapleyExperimentHarness for Q{i} (n={len(docs)} docs)..."
        )
    ...
    if accelerator_main.is_main_process:
        ...
        for size_key, num_s in m_samples_map.items():
            if (
                2 ** len(docs) < num_s and size_key != "L"
            ):  # Avoid sampling more than possible, except for a default
                actual_samples = max(
                    1, 2 ** len(docs) - 1 if 2 ** len(docs) > 0 else 1
                )  # Ensure at least 1 sample if possible
            else:
                actual_samples = num_s

            if actual_samples > 0:
                results_for_query[f"ContextCite{actual_samples}"] = (
                    harness.compute_contextcite_weights(
                        num_samples=actual_samples, sampling="uniform", seed=SEED
                    )
                )  # lasso_alpha=0.01,
                results_for_query[f"KernelShap{actual_samples}"] = (
                    harness.compute_contextcite_weights(
                        num_samples=actual_samples, sampling="kernel_shap", seed=SEED
                    )
                )  # lasso_alpha=0.01,
                ...
                results_for_query[f"BetaShap (U){actual_samples}"] = (
                    harness.compute_beta_shap(
                        num_iterations_max=T_iterations_map[size_key],
                        beta_a=0.5,
                        beta_b=0.5,
        ...
        df_save_results.loc[i, "id"] = [[flags]]
        ...
        if (
            exact_scores is not None
        ):  # proceed if the ground truth scores are avaialble for this query
            positive_exact_score = np.clip(
                exact_scores, a_min=0.0, a_max=None
            )  # FOR NDGC SCORE COMPUTATION
            for (
                method,
                approx_scores,
            ) in (
                results_for_query.items()
            ):  # Iterate over each method and its attributed scores

                if method != "Exact" and approx_scores is not None:
                    if len(approx_scores) == len(exact_scores):
                        if np.all(exact_scores == exact_scores[0]) or np.all(
                            approx_scores == approx_scores[0]
                        ):  # check if all elements in exact_scores / approx_scores are the same [1,1,1,1,1] => correlation becomes undefined in that case
                            ...
                            pearson_c = (
                                1.0 if np.allclose(exact_scores, approx_scores) else 0.0
                            )
                            spearman_c = (
                                1.0 if np.allclose(exact_scores, approx_scores) else 0.0
                            )
                            ...
                            pearson_c, _ = pearsonr(
                                exact_scores, approx_scores
                            )  # pearson measures linear correlation
                            spearman_c, _ = spearmanr(
                                exact_scores, approx_scores
                            )  # spearman measures rank correlation (how well the order matches)

                            exact_ranks = rankdata(
                                -np.array(exact_scores), method="average"
                            )  # rank scores with the smallest =1 and when there is a tie assign the average rank
                            approx_ranks = rankdata(
                                -np.array(approx_scores), method="average"
                            )
                            kendall_c, _ = kendalltau(
                                exact_ranks, approx_ranks
                            )  # return tau and pval (if pval is < 0.005 we can say that correlation is statistically significant)
                            precision_1 = precision_at_k(
                                exact_scores, approx_scores, k=1
                            )  # k = 3 by default
                            precision_3 = precision_at_k(
                                exact_scores, approx_scores, k=3
                            )  # k = 3 by default
                            precision_5 = precision_at_k(
                                exact_scores, approx_scores, k=5
                            )  # k = 3 by default
                        ...
                        print(
                            f"Score length mismatch for method {method} (Exact: {len(exact_scores)}, Approx: {len(approx_scores)}). Skipping metrics."
                        )
        else:
            print(
                f"    Skipping metric calculation for Q{i} as Exact Shapley was not computed or failed."
            )
    ...
    if torch.cuda.is_available():
        if accelerator_main.is_main_process:  # Print from one process
            print(
                f"Attempting to empty CUDA cache on rank {accelerator_main.process_index} after Q{i}"
            )
        torch.cuda.empty_cache()
        gc.collect()
        if accelerator_main.is_main_process:
            print(
                f"CUDA cache empty attempt complete on rank {accelerator_main.process_index}."
            )
    accelerator_main.wait_for_everyone()

# SAVE EXACT SHAP RESULTS FOR ANALYSIS
df_save_results.to_csv(
    f"Experiment_results_data/{DATASET_NAME}/{MODEL_NAME}.csv", index=False
)

# Save
# df_save_results.to_json(f"Experiment_results_data/{DATASET_NAME}/{MODEL_NAME}.json", orient="records", lines=True)
...
if accelerator_main.is_main_process:
    if all_metrics_data:
        ...
        average_metrics = (
            metrics_df_all_questions.groupby("Method")
            .agg(
        ...
        details_path = f"Experiment_metrics_data/{DATASET_NAME}/{MODEL_NAME}/shapley_rag_experiment_details.csv"
        summary_path = f"Experiment_metrics_data/{DATASET_NAME}/{MODEL_NAME}/shapley_rag_experiment_summary.csv"
...
if torch.distributed.is_available() and torch.distributed.is_initialized():
    if accelerator_main.is_local_main_process:
        print(
            f"Rank {accelerator_main.process_index} (Local Main): Manually destroying process group..."
        )
    torch.distributed.destroy_process_group()
    if accelerator_main.is_local_main_process:
        print(
            f"Rank {accelerator_main.process_index} (Local Main): Process group destroyed."
        )
else:
    if accelerator_main.is_local_main_process:
        print(
            f"Rank {accelerator_main.process_index} (Local Main): Distributed environment not initialized or not available, skipping destroy_process_group."
        )
