...
# data_type = input("What type of dataset to use (synergy, complementary, duplicate)? ")

csv_path = os.path.join(current_dir, f'../data/synthetic_data/{args.data_type}.csv')

df = pd.read_csv(csv_path)
df_save_results = pd.DataFrame(columns = ["query", "context", "provided_answer", "scoring", "doc_flags"])
print("Data Loaded!")
...
SEED = 42
DATASET_NAME = f"synthetic_data_{args.data_type}"
...
for i in tqdm(range(num_questions_to_run), desc="Processing Questions", disable=not accelerator_main.is_main_process):
    ...
    if accelerator_main.is_main_process:
        ...
        # df_save_results.loc[i, "doc_flags"] = [list(string.ascii_uppercase[:10])]
...
df_save_results.to_csv(f"../data/synthetic_data/results/results_{args.data_type}.csv", index=False)

# Save
df_save_results.to_json(f"../data/synthetic_data/results/results_{args.data_type}.json", orient="records", lines=True)
